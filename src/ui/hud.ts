...
export function mountHud(): void {
  ...
  if (!inventoryEl || !detailEl) throw new Error('HUD-Container fehlen im DOM.');
...
function renderInventory(el: HTMLElement, opts: InventoryOptions): void {
  ...
  if (nonFree.length > 0) {
    el.appendChild(sectionHeader('Gebaute Teilchen'));
    for (const [id, count] of nonFree) {
      const entity = getEntity(id);
      if (!entity) continue;
...
function inventoryRow(
  ...
  addBtn.disabled = availableCount(entity.id) <= inZone;
...
function zonePanel(zone: Record<string, number>): HTMLElement {
  ...
  if (entries.length === 0) {
    ...
    empty.textContent = 'Leer — füge Zutaten hinzu.';
